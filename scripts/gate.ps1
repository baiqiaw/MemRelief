...
Write-Host '=== 门禁 1/5：编译解决方案 ===' -ForegroundColor Cyan
...
Write-Host '=== 门禁 2/5：Core 引用检查（法-1） ===' -ForegroundColor Cyan
...
Write-Host '=== 门禁 3/5：Core 测试 + 覆盖率 ≥80% ===' -ForegroundColor Cyan
...
Write-Host '=== 门禁 4/5：App 测试 + 覆盖率 ≥80% ===' -ForegroundColor Cyan
dotnet test (Join-Path $repo 'tests\MemRelief.App.Tests') -c Debug --nologo `
    -p:Threshold=80 -p:ThresholdType=line -p:ThresholdStat=total
if ($LASTEXITCODE -ne 0) { Write-Host '[gate] App 测试/覆盖率门禁失败' -ForegroundColor Red; exit 1 }

# 验证台（T-21 起纳入机器门）：阈值同 80（total line），Include 由本命令注入（只统计 MemRelief.Bench）
Write-Host '=== 门禁 5/5：验证台测试 + 覆盖率 ≥80% ===' -ForegroundColor Cyan
dotnet test (Join-Path $repo 'tests\MemRelief.Bench.Tests') -c Debug --nologo `
    -p:CollectCoverage=true "-p:Include=[MemRelief.Bench]*" `
    -p:Threshold=80 -p:ThresholdType=line -p:ThresholdStat=total
if ($LASTEXITCODE -ne 0) { Write-Host '[gate] 验证台测试/覆盖率门禁失败' -ForegroundColor Red; exit 1 }
