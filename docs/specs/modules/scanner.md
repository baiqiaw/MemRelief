...
- **候选验签（两阶段，法级）**：`Task<ScanResult> CollectSignatures(ScanResult, ISet<int> candidatePids)` → 仅对候选执行 WinVerifyTrust 并回填签名字段（口径表 #9"仅候选执行"；系统目录核心命中且非 UWP 不验签直接按微软处理；缓存见 §6）。全候选 Unverifiable 时追加全局 SignalFailure（系统性失效探针，触发即全局保守降级，见 data-contracts §1.1 #35-1）。编排方在 `CandidateIds`（rules）之后调用（时序见 §4.3）。
- **采集概览**：`Task<MemoryOverview> SampleOverview()` → 三级通道梯：NtQuerySystemInformation 优先（含 standby）→ PDH 计数器兜底（commit/available/standby）→ GlobalMemoryStatusEx 终底（commit 用页面文件口径近似，Source=Degraded、standby 恒 null）；降级语义：standby 不可得 ⇔ StandbyBytes=null 且 Source=Degraded（T-05 实装，裁决见 [data-contracts §2](../interfaces/data-contracts.md)）；Detail 字段承载通道降级/兜底原因（生产端 Sampler 保证 Degraded 恒非空，issue #31，口径见 data-contracts §1.1）。
- **CPU 差分**：TakeSnapshot 采集段内首尾两次采样求差（口径表 #7），随快照输出。
