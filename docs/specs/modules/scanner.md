...
- **候选验签（两阶段，法级）**：`Task<ScanResult> CollectSignatures(ScanResult, ISet<int> candidatePids)` → 仅对候选执行 WinVerifyTrust 并回填签名字段（口径表 #9"仅候选执行"；系统目录核心命中且非 UWP 不验签直接按微软处理；缓存见 §6）。编排方在 `CandidateIds`（rules）之后调用（时序见 §4.3）。
